...
//			testingProcessClass.addTrees( testingProcess.getTreeList() );
			...
			if( testCaseArgs.get(TestingParam.KMEANWEIGTH.getArgCode()).equals("1") ) {
				...
				AnalysisTFIDF simpleAnalyzer4=new AnalysisTFIDF(tempClusterer.getWordModel(),validationClassClusterer.getFeatureList());
					   ...
					   clusterCenters += Integer.valueOf( testCaseArgs.get(TestingParam.KMEANSCLUSTERSIZESTEP.getArgCode()) ) )
		{
			...
			System.out.println("validation Cluster methods");
			validationMethodClusterer.doClusteringWithInputArgsExceptKmeansCenters( testCaseArgs , clusterCenters );
			...
			System.out.println("testing Cluster class");
			validationClassClusterer.doClusteringWithInputArgsExceptKmeansCenters( testCaseArgs , clusterCenters );
			...
			lineOfValidationClassClusterEval.add( clusterCenters , validationClassClusterer.getClusterEvaluation() );
			lineOfValidationMethodClusterEval.add( clusterCenters , validationMethodClusterer.getClusterEvaluation() );
			...
			AnalysisTFIDF validationTrainingMethod=new AnalysisTFIDF(validationMethodClusterer.getClusterFeatures(),tempClusterer.getWordModel());
			validationTrainingMethod.perfomTFIDF();
			AnalysisTFIDF validationTrainingClass=new AnalysisTFIDF(validationClassClusterer.getClusterFeatures(),tempClusterer.getWordModel());
			validationTrainingClass.perfomTFIDF();
